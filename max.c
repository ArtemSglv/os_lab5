...
/*static int setxattr(char *path, char *name, char *value, int flags)


static int do_setattr (char *path, char *name, char *value, int flags)
{
...
static void generate_tree() {
	...
	bar = add_directory_with_perm(root, "bar", 0755);				// dir
		baz = add_directory_with_perm(bar, "baz", 0744);			// dir
			readme = add_file_with_perm(baz, "readme.txt", 0777);	//file
